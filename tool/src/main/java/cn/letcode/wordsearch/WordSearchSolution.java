...
public class WordSearchSolution {
        ...
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[0].length; j++) {
                if (board[i][j] == chars[0]) {
                    ...
                    if (checkChar(board, i, j, chars, 0, sets)) {
                        return true;
                    }
                }
            }
    ...
    private boolean checkChar(char[][] board, int i, int j, char[] chars, int index, Set<String> sets) {
        if (board[i][j] == chars[index] && index == chars.length - 1) {
            return true;
        } else if (board[i][j] != chars[index]) {
            return false;
        } else {
            sets.add(i + "" + j);
        }
        boolean flag = false;
        if (i > 0 && !sets.contains((i - 1) + "" + j) && checkChar(board, i - 1, j, chars, index + 1, new HashSet<>(sets))) {
            return true;
        }
        if (j > 0 && !sets.contains((i) + "" + (j - 1)) && checkChar(board, i, j - 1, chars, index + 1, new HashSet<>(sets))) {
            return true;
        }
        if (i >= 0 && !sets.contains((i + 1) + "" + j) && i < board.length - 1 && checkChar(board, i + 1, j, chars, index + 1, new HashSet<>(sets))) {
            return true;
        }
        if (j >= 0 && !sets.contains((i) + "" + (j + 1)) && j < board[0].length - 1 && checkChar(board, i, j + 1, chars, index + 1, new HashSet<>(sets))) {
            return true;
        }
        return flag;
    }
...
//                };
        ...
        s="baaaaa";
        boolean abcced = new WordSearchSolution().exist(board, s);
