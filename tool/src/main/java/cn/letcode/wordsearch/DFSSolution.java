...
public class DFSSolution {
    public boolean exist(char[][] board, String word) {
        ...
        // direction of top, right, down, left in (x,y) coordinate
        int[][] move = {{-1, 0}, {0, 1}, {1, 0}, {0, -1}};
        for (int i=0; i<row; i++) {
            for (int j=0; j<col; j++) {
                if(existHelper(board, word, i, j, 0, move)) {
                    return true;
                }
            }
        }
    ...
    private boolean existHelper(char[][]board, String word, int i, int j, int ptr, int[][] move) {
        ...
        if (board[i][j] == word.charAt(ptr)) {
            ...
            board[i][j] = '$';
            for (int[] m : move) {
                if (existHelper(board, word, i+m[0], j+m[1], ptr+1, move)) {
                    return true;
                }
            }
            board[i][j] = tmp;
...
//                };
        ...
        //s="baaaaa";
        boolean abcced = new DFSSolution().exist(board, s);
